...
(function () {
  ...
  var renderElementList = function (elementList, template) {
    var list = '';
    for (var i = 0; i < elementList.length; i++) {
      list += template.replace(/\{\{x\}\}/g, elementList[i]);
    }
  ...
  var renderCard = function (element, announcementData) {
    ...
    element.querySelector('.popup__features').innerHTML = renderElementList(announcementData.offer.features, FEATURE_TEMPLATE);
    element.querySelector('.popup__description').textContent = announcementData.offer.description;
    element.querySelector('.popup__photos').innerHTML = renderElementList(announcementData.offer.photos, PHOTO_TEMPLATE);
